      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Catalog albums. Open an album to reorder tracks on its collection
            page.
          </p>
        </div>
        <Button asChild>
                ...
                <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-sm text-muted-foreground">
                      ...
                      {a.visibility}
                      {a.card_badge_label?.trim() ? (
                        <>
                          <span className="mx-2">·</span>
                          Badge: {a.card_badge_label.trim()}
                        </>
                      ) : null}
                    </p>
                  </div>
                  <div className="flex shrink-0 flex-wrap gap-2">
                    <Button size="sm" asChild>
                      <Link href={`/admin/albums/${a.id}`}>Track order</Link>
                    </Button>
                    <Button size="sm" variant="secondary" asChild>
                      <Link
                        href={`/music/collections/${encodeURIComponent(a.slug)}`}
                        target="_blank"
