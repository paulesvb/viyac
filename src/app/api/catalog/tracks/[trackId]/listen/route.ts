...
export async function POST(
  ...
  try {
    ...
    let listenerId: string = jar.get(ANON_LISTENER_COOKIE)?.value?.trim() ?? '';
    if (!isLikelyUuid(listenerId)) {
    ...
    if (readError) {
      console.error('[catalog/tracks/listen] anonymous read', readError);
      return NextResponse.json({ error: readError.message }, { status: 500 });
    }
    ...
    if (upsertError) {
      console.error('[catalog/tracks/listen] anonymous upsert', upsertError);
