...
export function CreateTrackForm({ albums, genesisOriginals }: Props) {
  ...
  const [storageTrackPath, setStorageTrackPath] = useState('');
  const [autoTrackPath, setAutoTrackPath] = useState(true);
  const [autoWaveformPath, setAutoWaveformPath] = useState(true);

  const normalizeStoragePath = useCallback((value: string): string => {
    return value.trim().replace(/^\/+|\/+$/g, '');
  }, []);

  const buildTrackPathFromStorage = useCallback(
    (value: string): string => {
      const normalized = normalizeStoragePath(value);
      return normalized ? `${normalized}/index.m3u8` : '';
    },
    [normalizeStoragePath],
  );

  const buildWaveformPathFromStorage = useCallback(
    (value: string): string => {
      const normalized = normalizeStoragePath(value);
      return normalized ? `${normalized}/waveform.json` : '';
    },
    [normalizeStoragePath],
  );

  const onStorageTrackPathChange = useCallback(
    (value: string) => {
      setStorageTrackPath(value);
      if (autoTrackPath) {
        setTrackPath(buildTrackPathFromStorage(value));
      }
      if (autoWaveformPath) {
        setWaveformJsonVaultPath(buildWaveformPathFromStorage(value));
      }
    },
    [
      autoTrackPath,
      autoWaveformPath,
      buildTrackPathFromStorage,
      buildWaveformPathFromStorage,
    ],
  );

  const onTrackPathChange = useCallback(
    (value: string) => {
      setTrackPath(value);
      const derived = buildTrackPathFromStorage(storageTrackPath);
      setAutoTrackPath(value.trim() === derived);
    },
    [buildTrackPathFromStorage, storageTrackPath],
  );

  const onWaveformPathChange = useCallback(
    (value: string) => {
      setWaveformJsonVaultPath(value);
      const derived = buildWaveformPathFromStorage(storageTrackPath);
      setAutoWaveformPath(value.trim() === derived);
    },
    [buildWaveformPathFromStorage, storageTrackPath],
  );
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-2xl space-y-8">
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="storage_track_path">Storage track path (optional)</Label>
          <Input
            id="storage_track_path"
            value={storageTrackPath}
            onChange={(e) => onStorageTrackPathChange(e.target.value)}
            placeholder="folder/track-name"
            className="font-mono text-sm"
          />
          <p className="text-xs text-muted-foreground">
            Auto-fills vault track path as{' '}
            <code className="text-[11px]">index.m3u8</code> and waveform path as{' '}
            <code className="text-[11px]">waveform.json</code>. Manual edits are
            preserved.
          </p>
        </div>
        <div className="space-y-2">
          ...
          <Input
            id="track_path"
            value={trackPath}
            onChange={(e) => onTrackPathChange(e.target.value)}
            required
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            New rows default to <span className="font-medium">private</span> in the database.
            Home only lists tracks that are public/unlisted, or sit on a public/unlisted album,
            and that opt in with “Show on signed-in Home” below.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Label htmlFor="create_featured" className="font-normal">
            Featured (prefer this track in Home lists and marquee when shown on Home)
          </Label>
        </div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="h-4 w-4 rounded border-input"
          />
          <Label htmlFor="create_show_home" className="font-normal">
            Show on signed-in Home
          </Label>
        </div>
        <p className="text-xs text-muted-foreground">
          Off by default. When unchecked, the track does not appear on Home even if public;
          direct links and album pages still work when visibility allows.
        </p>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => onWaveformPathChange(e.target.value)}
