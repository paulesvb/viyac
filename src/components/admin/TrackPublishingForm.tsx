...
export function TrackPublishingForm({
  ...
  const [showHome, setShowHome] = useState(initial.show_home);
    ...
    <form onSubmit={onSubmit} className="max-w-md space-y-6">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="h-4 w-4 rounded border-input"
        />
        <Label htmlFor="show_home" className="font-normal">
          Show on signed-in Home (featured player and More tracks)
        </Label>
      </div>
      <p className="text-xs text-muted-foreground">
        Off by default. When unchecked, the track stays out of Home entirely;
        direct links, albums, and catalog pages still work when visibility allows.
      </p>
