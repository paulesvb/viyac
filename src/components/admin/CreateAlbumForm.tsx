...
export function CreateAlbumForm() {
  ...
  const [cardBadgeLabel, setCardBadgeLabel] = useState('');
  ...
  const onSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      const payload: CreateAlbumInput = {
        ...
        card_badge_label: cardBadgeLabel,
      };
    ...
    [title, slug, visibility, coverPath, cardBadgeLabel, router],
  );
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-lg space-y-5">
      ...
      <div className="space-y-2">
        <Label htmlFor="card_badge_label">Card badge label (optional)</Label>
        <Input
          id="card_badge_label"
          value={cardBadgeLabel}
          onChange={(e) => setCardBadgeLabel(e.target.value)}
          placeholder="e.g. New · Genesis · Inner Circle"
          maxLength={48}
        />
        <p className="text-xs text-muted-foreground">
          Short copy on the Home collection card. Leave empty to hide the badge.
        </p>
      </div>
