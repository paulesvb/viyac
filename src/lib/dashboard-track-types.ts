...
export type DashboardTrack = {
  ...
  /**
   * When false, hidden from signed-in Home (featured and More tracks).
  ...
  duration_ms?: number;
  /** Shared page: number of other users (excluding current viewer) with explicit grants. */
  shared_with_others_count?: number;
  /** Shared page: display labels for other users with explicit grants. */
  shared_with_others_names?: string[];
  /** Catalog: this track is a cover of a Genesis original (`api.tracks.is_cover`). */
  is_cover?: boolean;
  /** Slug for the Genesis original when the viewer can access it (music page link). */
  original_genesis_slug?: string;
  /** Title of the Genesis original when linked. */
  original_genesis_title?: string;
};
