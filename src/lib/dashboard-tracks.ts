...
import {
  coLocatedVaultFile,
  resolveVaultSiblingPath,
} from '@/lib/vault-stream';
import type { VaultTrackData } from '@/components/VaultPlayer';

export type { DashboardTrack };

/** Same identity check used for player vs list row (catalog UUID, else slug). */
export function dashboardTracksMatch(
  a: DashboardTrack,
  b: DashboardTrack,
): boolean {
  const aId = a.catalog_track_id?.trim();
  const bId = b.catalog_track_id?.trim();
...
export function getOtherDashboardTracksFromList(
  ...
  return tracks.filter((t) => {
    if (t.show_home === false) return false;
...
/**
 * Home “More tracks” — all tracks eligible for Home, including the one in the player.
 */
export function getHomeMoreTracksExcludingPlaying(
  tracks: DashboardTrack[],
  _playing: DashboardTrack | null,
): DashboardTrack[] {
  return tracks.filter((t) => t.show_home !== false);
}

/** Album page list: same order as `tracks`, including the track in the player. */
export function getAlbumTracksExcludingPlaying(
  tracks: DashboardTrack[],
  _playing: DashboardTrack | null,
): DashboardTrack[] {
  return tracks;
...
export function toVaultTrackData(track: DashboardTrack): VaultTrackData {
  const wfVaultRaw = track.waveform_json_vault_path?.trim();
  const wfVault =
    resolveVaultSiblingPath(track.track_path, wfVaultRaw, 'waveform.json') ??
    wfVaultRaw ??
    coLocatedVaultFile(track.track_path, 'waveform.json') ??
    undefined;
  const wfPublic = track.waveform_json_path?.trim();
  const bgVideoRaw = track.vault_background_video_path?.trim();
  const bgVideo =
    resolveVaultSiblingPath(track.track_path, bgVideoRaw, 'video-loop.mp4') ??
    bgVideoRaw ??
    coLocatedVaultFile(track.track_path, 'video-loop.mp4') ??
    undefined;
  return {
    ...
    ...(wfVault
      ...
      : wfPublic
        ...
        : {}),
    ...(track.vault_background_video_path?.trim() || bgVideo
      ? {
          vault_background_video_path:
            bgVideo ?? track.vault_background_video_path!.trim(),
        }
      : {}),
