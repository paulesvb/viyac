    ...
    if (albumMode === 'album' && isCatalogAlbumId(albumIdForLink)) {
      ...
      if (linkErr) {
        ...
        for (const s of slugsBefore) {
          revalidatePath(`/music/collections/${encodeURIComponent(s)}`);
        }
...
export async function reorderAlbumTracks(
  ...
  if (uniqueIds.length !== orderedTrackIds.length) {
    return { ok: false, error: 'Duplicate track ids in order list.' };
  }
  if (!uniqueIds.every((id) => isCatalogTrackId(id))) {
    return { ok: false, error: 'Invalid track id in order list.' };
  }
  ...
  if (albumError || !albumRow) {
    return {
      ok: false,
      error: albumError
        ? formatCatalogPostgrestError(albumError.message, 'Album not found.')
        : 'Album not found.',
    };
  }
  ...
  if (linksError) {
    return {
      ok: false,
      error: formatCatalogPostgrestError(linksError.message, 'Could not load tracks.'),
    };
  }

  const existingIds = new Set(
    (existingLinks ?? []).map((row) => (row as { track_id: string }).track_id),
  );
  if (existingIds.size !== uniqueIds.length) {
    return { ok: false, error: 'Track list does not match this album.' };
  }
  ...
  const updates = uniqueIds.map((trackId, index) =>
    supabase
      .from('album_tracks')
  ...
  if (failed?.error) {
    console.error('[reorderAlbumTracks]', failed.error);
    return {
      ok: false,
      error: formatCatalogPostgrestError(failed.error.message, 'Could not save order.'),
    };
  }

  const slug = (albumRow as { slug: string }).slug.trim();
  if (slug) {
    revalidatePath(`/music/collections/${encodeURIComponent(slug)}`);
    revalidatePath(`/es/music/collections/${encodeURIComponent(slug)}`);
  }
  revalidatePath('/admin/albums');
