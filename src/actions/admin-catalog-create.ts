...
export type CreateTrackInput = {
  ...
  featured: boolean;
  /** Opt-in to signed-in Home (featured + More tracks). */
  show_home: boolean;
};
...
export async function createCatalogTrack(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      show_home: Boolean(input.show_home),
  ...
  if (albumMode === 'album') {
    ...
    if (albumSlugForRevalidate) {
      revalidatePath(
        `/music/collections/${encodeURIComponent(albumSlugForRevalidate)}`,
      );
    }
...
export async function createCatalogAlbum(
  ...
  const badge = trimOrEmpty(input.card_badge_label);
